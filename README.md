...
C:\Users\user>
 ```
## Mode 0 
## Find Passphrases and Privat keys from a file
...
### Privat keys from a file
 - For CPU (NORMAL) ```LostCoins.exe -t 1 -f test.bin -r 0 -s Base-passphrases.txt -z keys``` 
 - For CPU (SLOW) ```LostCoins.exe -t 1 -f test.bin -r 0 -s Base-passphrases.txt -z keys -d 0```
 - For CPU (Very SLOW)  ```LostCoins.exe -t 1 -f test.bin -r 0 -s Base-passphrases.txt -z keys -d 1```
